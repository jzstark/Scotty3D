...
std::optional<Halfedge_Mesh::EdgeRef> Halfedge_Mesh::flip_edge(EdgeRef e) {
	//A2L1: Flip Edge
	//Ref #1: https://gitlab.vci.rwth-aachen.de:9000/OpenMesh/OpenMesh/-/blob/master/src/OpenMesh/Core/Mesh/TriConnectivity.cc
	//Ref #2: https://stanford-cs248.github.io/Cardinal3D/meshedit/edge_flip
	// However, these references only take care about "both triangle" situation
	// We need to consider the general case where both shapes are not triangles
    ...
    HalfedgeRef h3 = h1->next; //start from h2; h3 = h0->prev
	while (h3->next != h0) {
        h3 = h3->next;
    }
    HalfedgeRef h4 = h0->twin;
    HalfedgeRef h5 = h4->next;
	HalfedgeRef h7 = h4->next; // start from h5; h7 = h4->prev
	while (h7->next != h4) {
        h7 = h7->next;
    }
	HalfedgeRef h6 = h4->next; // start from h5; h6 = h7->prev 
	while (h6->next != h7) {
        h6 = h6->next;
    }
	...
	// change the HE's vertices
	h0->vertex = v2;
	h4->vertex = v3;

	// Change the next of HE's
	h1->next = h4;
	h4->next = h7;
	h7->next = h1;
	h0->next = h2;
	...
	// No need to change the edge and twin of HE's
	
	// Change the halfedge of two vertices that are connected to e 
	v0->halfedge = h5;
	v1->halfedge = h1;
