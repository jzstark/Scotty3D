...
std::pair<Ray, float> Camera::sample_ray(RNG &rng, uint32_t px, uint32_t py) {
    ...
    Vec2 sensor_plane_pos = Vec2(
        (sensor_pixel.x / float(film.width) - 0.5f) * w,
        (sensor_pixel.y / float(film.height) - 0.5f) * h
    );
	...
	ray.dir = Vec3(sensor_plane_pos.x, sensor_plane_pos.y, -1.0f).unit();
